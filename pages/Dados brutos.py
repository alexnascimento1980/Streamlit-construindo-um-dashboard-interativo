...
with st.sidebar.expander('Nome do Produto'):
    produtos = st.multiselect('Selecione os produtos que deseja visualizar',
                              dados['Produto'].unique(),
                              dados['Produto'].unique()
                              )

with st.sidebar.expander('Preço do Produto'):
    preco = st.slider('Selecione o preço que deseja visualizar',
                      0,
                      5000,
                      (0, 5000)
                      )
...
with st.sidebar.expander('Data da Compra'):
    data_compra = st.date_input('Selecione a data da compra que deseja visualizar',
